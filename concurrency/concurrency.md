...
this operation is _blocking_ - it makes us wait for it to finish. An operation
that does not block in Go will run in a separate _process_ called a _goroutine_.
...
call, we often use _anonymous functions_ when we want to start a goroutine. An
